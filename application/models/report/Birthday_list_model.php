...
class birthday_list_model extends my_model{
    public function __construct(){ parent::__construct('report', 'birthday_list'); }
    public function get_record(){
        ...
        if(isset($_REQUEST['_birth_date_from'])){
            if($_REQUEST['_birth_date_from'] != ''){
                $subsql .=" AND DATE_FORMAT(customer.customer_birth_date, '%m-%d') >= '".date('m-d', strtotime($_REQUEST['_birth_date_from']))."'";
                $record['filter']['_birth_date_from'] = $_REQUEST['_birth_date_from'];
            }
        }else{
            $subsql .=" AND DATE_FORMAT(customer.customer_birth_date, '%m-%d') >= '".date('m-d')."'";
        }
        if(isset($_REQUEST['_birth_date_to'])){
            if($_REQUEST['_birth_date_to'] != ''){
                $subsql .=" AND DATE_FORMAT(customer.customer_birth_date, '%m-%d') <= '".date('m-d', strtotime($_REQUEST['_birth_date_to']))."'";
                $record['filter']['_birth_date_to'] = $_REQUEST['_birth_date_to'];
            }
        }else{
            $subsql .=" AND DATE_FORMAT(customer.customer_birth_date, '%m-%d') <= '".date('m-d')."'";
        }
        $query="SELECT 
                ...
                DATE_FORMAT(customer.customer_birth_date, '%d-%m-%Y') as birth_date,
                IF(customer.customer_birth_date = '', 0, customer.customer_birth_date) as customer_birth_date,
                '' as created_at
                ...
                AND (customer.customer_birth_date > 0)
                $subsql
        ...
        if(!empty($data)){
            foreach ($data as $key => $value) {
                array_push($record['data'], [
                                                ...
                                                'customer_birth_date'   => (int)strtotime($value['customer_birth_date']),
