    ...
    <?php 
        if(!empty($data)): 
            foreach ($data as $key => $value):
                $id = encrypt_decrypt("encrypt", $value['karigar_id'], SECRET_KEY);
    ?>

                <tr class="<?php echo $value['karigar_status'] == 0 ? 'text-danger' : '' ?>">
                    ...
                    <?php if(in_array('delete', $action_data)): ?>
                        <td width="3%">
                            <a 
                                type="button" 
                                class="btn btn-sm btn-primary" 
                                onclick='karigar_remove(<?php echo json_encode($value); ?>);'
