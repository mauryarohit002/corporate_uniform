    ...
    <?php 
        if(!empty($data)): 
            foreach ($data as $key => $value):
                $id = encrypt_decrypt("encrypt", $value['design_id'], SECRET_KEY);
    ?>

                <tr class="<?php echo $value['design_status'] == 0 ? 'text-danger' : '' ?>">
                    <td width="3%"><?php echo $key+1; ?></td>
                    <td width="10%"><?php echo $value['design_name']; ?></td>
                    <td width="10%"><?php echo $value['supplier_name']; ?></td>
                    <td width="10%"><?php echo $value['design_status'] == 1 ? 'active' : 'inactive'; ?></td>
                    <?php if(in_array('read', $action_data)): ?>
                        <td width="3%">
                            <a 
                                type="button" 
                                class="btn btn-sm btn-primary" 
                                onclick='design_popup(<?php echo json_encode(["action" => "read", "id" => $value["design_id"]]); ?>)'>
                    ...
                    <?php if(in_array('edit', $action_data)): ?>
                        <td width="3%">
                            <a 
                                type="button" 
                                class="btn btn-sm btn-primary" 
                                onclick='design_popup(<?php echo json_encode(["action" => "edit", "id" => $value["design_id"]]); ?>)'>
                        ...
                        <td width="3%">
                            ...
                            <?php else: ?>
                                <a 
                                    type="button" 
                                    class="btn btn-sm btn-primary"
                                    onclick='design_remove(<?php echo json_encode($value); ?>);'
                                >
