...
class Readymade_qrcode_stock extends my_controller{
	protected $menu;
    protected $sub_menu;
	public function __construct(){
		$this->menu     = 'report';
        $this->sub_menu = 'readymade_qrcode_stock';
	...
	public function excel(){	 
		...
		$this->excel->getActiveSheet()->setTitle("Readymade Barcode Stock");
		$this->excel->getActiveSheet()->setCellValue('A'.$line_no, "Readymade Barcode Stock");			 
		...
		$this->excel->getActiveSheet()->getStyle('N'.$line_no.":P".$line_no)->getFont()->setBold( true );	
		 ...
		 if(!empty($record['data'])){
				...
				$this->excel->getActiveSheet()->SetCellValue('F'.$line_no, 'PRODUCT');

				$this->excel->getActiveSheet()->getColumnDimension('G')->setAutoSize(true);	 
				$this->excel->getActiveSheet()->SetCellValue('G'.$line_no, 'DESIGN');
				...
				$this->excel->getActiveSheet()->SetCellValue('J'.$line_no, 'SIZE');
			 ...
			 foreach ($record['data'] as $key => $value) {
				    ...
				    $this->excel->getActiveSheet()->getColumnDimension('F')->setAutoSize(true);	 
				    $this->excel->getActiveSheet()->SetCellValue('F'.$line_no, $value['product_name']);
				    ...
				    $this->excel->getActiveSheet()->getColumnDimension('J')->setAutoSize(true);	 
				    $this->excel->getActiveSheet()->SetCellValue('J'.$line_no, $value['size_name']);
				    ...
				    $this->excel->getActiveSheet()->getColumnDimension('N')->setAutoSize(true);	 
				    $this->excel->getActiveSheet()->SetCellValue('N'.$line_no, $value['prmt_qty']);

				    $this->excel->getActiveSheet()->getColumnDimension('O')->setAutoSize(true);	 
				    $this->excel->getActiveSheet()->SetCellValue('O'.$line_no, $value['ot_qty']);

				    $this->excel->getActiveSheet()->getColumnDimension('P')->setAutoSize(true);	 
				    $this->excel->getActiveSheet()->SetCellValue('P'.$line_no, $value['bal_qty']);
			 ...
			 $this->excel->getActiveSheet()->SetCellValue('N'.$line_no, $record['totals']['prmt_qty']);

			 $this->excel->getActiveSheet()->getColumnDimension('O')->setAutoSize(true);
			 $this->excel->getActiveSheet()->getStyle('O'.$line_no)->getFont()->setBold( true );	
			 $this->excel->getActiveSheet()->SetCellValue('O'.$line_no, $record['totals']['ot_qty']);

			 $this->excel->getActiveSheet()->getColumnDimension('P')->setAutoSize(true);
			 $this->excel->getActiveSheet()->getStyle('P'.$line_no)->getFont()->setBold( true );	
			 $this->excel->getActiveSheet()->SetCellValue('P'.$line_no, $record['totals']['bal_qty']);
		 ...
		 $filename='readymade_barcode_summary_'.time().'.xlsx';
