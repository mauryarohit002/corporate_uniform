...
class design extends my_controller{
	...
	public function add_update(){
		...
		$post_data[$this->sub_menu.'_image'] 		= $result['data']; 
		...
		$temp = $this->db_operations->get_record($this->sub_menu.'_master', [
																	...
																	$this->sub_menu.'_supplier_id' 	=> $post_data[$this->sub_menu.'_supplier_id'],
																]);
		...
		if($id == 0){
			...
			if($this->model->isExist($id)){
				if($prev_data[0][$this->sub_menu.'_supplier_id'] != $post_data[$this->sub_menu.'_supplier_id']){
					$this->db->trans_rollback();
					return ['msg' => 'Not allowed to change supplier.'];
				}
			}
