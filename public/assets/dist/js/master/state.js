$(document).ready(function () {
  $("#_state_name").select2(
    select2_default({
      url: `master/state/get_select2/_state_name`,
      placeholder: "state",
    })
  );
});
...
const state_popup = (id, action = "add", field = undefined) => {
  const sub_menu = "state";
...
const remove_state_notifier = (sub_menu) => {
...
const add_update_state = (id, field) => {
  ...
  const sub_menu = "state";
  let check = true;
  remove_state_notifier(sub_menu);
...
const remove_state = (data) => {
  const path = `${link}/${sub_link}/handler`;
  const form_data = { func: "remove", id: data.state_id };
  let html = `<table class="table table-sm table-hover text-uppercase">
                <tbody>
                  ...
                  <tr>
                    ...
                    <td width="70%">
                      ${data.state_status == 1 ? "active" : "inactive"}
                    </td>
                  </tr>
                </tbody>
            </table>`;
