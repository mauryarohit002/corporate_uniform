$(document).ready(function () {
  $("#_country_name").select2(
    select2_default({
      url: `master/country/get_select2/_country_name`,
      placeholder: "country",
    })
  );
});
const set_country_field = (id) => {
  const sub_menu = "country";
...
const country_popup = (id, action = "add", field = undefined) => {
  const sub_menu = "country";
...
const remove_country_notifier = (sub_menu) => {
...
const add_update_country = (id, field) => {
  ...
  const sub_menu = "country";
  let check = true;
  remove_country_notifier(sub_menu);
...
const remove_country = (data) => {
  const path = `${link}/${sub_link}/handler`;
  const form_data = { func: "remove", id: data.country_id };
  let html = `<table class="table table-sm table-hover text-uppercase">
                <tbody>
                  <tr>
                    <td class="font-weight-bold" width="30%" align="right">country : </td>
                    <td width="70%">${data.country_name}</td>
                  </tr>
                  <tr>
                    <td class="font-weight-bold" width="30%" align="right">STATUS : </td>
                    <td width="70%">
                      ${data.country_status == 1 ? "active" : "inactive"}
                    </td>
                  </tr>
                </tbody>
            </table>`;
