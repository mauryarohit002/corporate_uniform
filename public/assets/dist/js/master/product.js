$(document).ready(function () {});
let pincode_cnt = 1;
const set_product_field = (id) => {
  const path = `master/product/handler`;
  const form_data = { func: "get_data", id };
  ajaxCall(
    "POST",
    path,
    form_data,
    "JSON",
    (resp) => {
      if (handle_response(resp)) {
        const { data, msg } = resp;
        if (data && data.length != 0) {
          $(`#product_name`).val(data[0][`product_name`]);
          $(`#product_specification`).val(data[0][`product_specification`]);
          
          $(`#product_status`).bootstrapToggle(
            data[0][`product_status`] == 1 ? "on" : "off"
          );
          $("#popup_modal_sm").modal("show");
        }
      }
    },
    (errmsg) => {}
  );
};
const product_popup = (args) => {
  const { action = "add", id = 0, field = undefined } = args;
  let title = `<div class="col-12 col-sm-12 col-md-12 col-lg-12">
                  <p class="text-uppercase text-center font-weight-bold">${action} product</p>
                </div>`;
  let data = `<form class="form-horizontal" id="product_form" onsubmit="add_update_product(${id}, ${field})">              
                <div class="row pt-1">
                  <div class="col-12">
                    <div class="d-flex flex-wrap form-group floating-form">
                      <div class="col-12 col-sm-12 col-md-12 col-lg-12 floating-label">
                        <input 
                          type="text" 
                          class="form-control floating-input" 
                          id="product_name" 
                          name="product_name" 
                          onkeyup="validate_textfield(this, ${true})" 
                          placeholder=" " 
                          autocomplete="off" 
                        />   
                        <label class="text-uppercase">product <span class="text-danger">*</span></label>
                        <small class="form-text text-muted helper-text" id="product_name_msg"></small>
                      </div>
                    
                    
                      ${
                        field == undefined
                          ? `<div class="col-12 col-sm-12 col-md-4 col-lg-4 floating-label">
                              <input 
                                type="checkbox" 
                                id="product_status" 
                                name="product_status" 
                                data-toggle="toggle" 
                                data-on="ACTIVE" 
                                data-off="INACTIVE" 
                                data-onstyle="primary" 
                                data-offstyle="primary" 
                                data-width="100" 
                                data-size="normal" 
                                checked
                              />
                            </div>`
                          : `<input type="hidden" name="product_status" value="1">`
                      }
                    </div>              
                  </div>              
                </div>              
              </form>`;

  let btn = `<button 
              type="button" 
              class="btn btn-sm btn-primary" 
              id="sbt_btn" 
              onclick="add_update_product(${id}, ${field})" 
              style="width:15%;"
              ${id == 0 && "disabled"}
            >
              <div class="stage d-none"><div class="dot-flashing"></div></div>
              <div class="dot-text text-primary text-uppercase">${action}</div>
            </button>
            <button type="button" class="btn btn-sm btn-primary" data-dismiss="modal">CANCEL</button>`;

  $(".modal-title-sm").html(title);
  $(".modal-body-sm").html(data);
  $(".modal-footer-sm").html(action == "read" ? "" : btn);
  if (id == 0) {
    $("#popup_modal_sm").modal("show");
    $(`#product_status`).bootstrapToggle();
    setTimeout(() => {
      $(`#product_name`).focus();
    }, RELOAD_TIME);
  } else {
    set_product_field(id);
  }

  $("#product_category_id").select2(
    select2_default({
      url: `master/readymade_category/get_select2/_id`,
      placeholder: "SELECT",
      param: true,
    })
  );
};
const remove_product_notifier = () => {
  notifier(`product_name`);
};
const add_update_product = (id, field) => {
  event.preventDefault();
  let check = true;
  remove_product_notifier();
  if ($(`#product_name`).val() == "") {
    notifier(`product_name`, "Required");
    check = false;
  }
  if (!check) {
    toastr.error("You forgot to enter some information.", "Oh snap!!!", {
      closeButton: true,
      progressBar: true,
      preventDuplicates: true,
    });
    $("body, html").animate({ scrollTop: 0 }, 1000);
  } else {
    let path = `master/product/handler`;
    let form_data = $(`#product_form`).serialize();
    form_data += `&func=add_update&id=${id}`;
    ajaxCall(
      "POST",
      path,
      form_data,
      "JSON",
      (resp) => {
        if (handle_response(resp)) {
          const { data, msg } = resp;
          if (id == 0) {
            if (field != undefined) {
              $("#popup_modal_sm").modal("hide");
              // refresh_dropdown_select2(data, field);
            } else {
              $(`#product_name`).val("").focus();
              remove_product_notifier();
            }
          } else {
            $("#popup_modal_sm").modal("hide");
          }
          toastr.success("", msg, { closeButton: true, progressBar: true });
          $("body, html").animate({ scrollTop: 0 }, 1000);
        }
      },
      (errmsg) => {}
    );
  }
};
const product_remove = (data) => {
  const path = `${link}/${sub_link}/handler`;
  const form_data = { func: "remove", id: data.product_id };
  let html = `<table class="table table-sm table-hover text-uppercase">
                  <tbody>
                      <tr>
                          <td class="font-weight-bold" width="30%" align="right">product : </td>
                          <td width="70%">${data.product_name}</td>
                      </tr>
                      <tr>
                          <td class="font-weight-bold" width="30%" align="right">specification : </td>
                          <td width="70%">${data.product_specification}</td>
                      </tr>
                   
                      <tr>
                          <td class="font-weight-bold" width="30%" align="right">STATUS : </td>
                          <td width="70%">
                            ${data.product_status == 1 ? "active" : "inactive"}
                          </td>
                      </tr>
                  </tbody>
              </table>`;
  remove_datav3({ path, form_data, html });
};
